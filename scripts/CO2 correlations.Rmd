...
Plot pH and CO2 linear regression by tank density 
...
Plot CO2 versus temperature linear regression

```{r}
t_co2_bare <- ggplot(aligned, aes(temp_c_Bare, co2_5)) + 
  geom_point() + 
  geom_smooth(method = "lm", color="blue") + 
  labs(title = "Zero Density Eelgrass", x="", y="") +
  theme_set(theme_bw()) + theme(plot.title = element_text(hjust = 0.5, size = 12))

t_co2_eelgrass <- ggplot(aligned, aes(temp_c_Eelgrass, co2_3))+
  geom_point()+
  geom_smooth(method='lm', color="green")+
  labs(title = "High Density Eelgrass", x="", y="") +
  theme_set(theme_bw()) + theme(plot.title = element_text(hjust = 0.5, size = 12))

t_co2 <- grid.arrange(t_co2_bare, t_co2_eelgrass, nrow = 1, top = "CO2 versus Temperature", 
                       left = "Carbon Dioxide Concentrations (ppm)", 
                       bottom = "Temperature (C)")

ggsave("co2_t.png", t_co2, width=8, height=4)
...
ggplot(df, aes(density, co2, fill = density)) +
  ...
  geom_hline (yintercept = 421, linetype="dashed", width = 150, color = 'red') +
  stat_compare_means(aes(label = ..p.signif..), comparisons = compare_density) + 
  annotate("text", x= "Zero", y = 350, label = "Atmospheric", vjust = -0.5)
...
Diurnal cycles – plotting with sensors – do, pH 
