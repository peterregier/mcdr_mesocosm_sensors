...
csense_start <- "2023-06-01" # changing because we want to capture more events for correlations
...
exo_files <- tibble(file = list.files(path = exo_directory, full.names = T)) %>% 
  filter(grepl("230626|230724|230728|230807|230821|230926", file))
...
csense_formatted <- csense_trim %>% 
  ...
  mutate(tank = str_to_sentence(str_remove_all(name, "co2_mv_"))) %>% 
  #rename("co2_ppm" = value) %>% 
  rename("co2_mv" = value) %>% 
  select(-name)

df1 <- full_join(exo_trim, csense_formatted, by = c("datetime", "tank")) %>% 
  mutate(datetime = as.character(datetime))

write_csv(df1, "../data/240207_timeseries_raw.csv")
```

```{r}
ggplot(df1, aes(do_perc, co2_mv, color = tank)) + 
  geom_point()
```
